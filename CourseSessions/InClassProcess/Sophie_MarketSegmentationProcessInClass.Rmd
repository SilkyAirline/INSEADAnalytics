---
title: "Boats (A) A Segmentation Case"
author: "A. Aabye, M. Bodenwinkler, S. Drouin, L. Paulet, J. Regele and A. Ribeiro"
output:
  word_document:
    toc: yes
  html_document:
...
#look for correlations above 0.5 in absolute value
library(dplyr)
cor_df <- as.data.frame(as.table(thecor))
cor_high <- cor_df %>%  arrange(desc(Freq)) %>% filter(Freq>0.5)
```
...
* In absolute value, there are a few questions that have a high correlation (i.e. higher than 0.5). In particular, it is not surprising to see a high correlation between **Q1.13** and **Q1.14** *(corr = 0.64)*. Someone who buys the best boat might also buy the best accessories. This suggests that these two attributes could be combined.
* Similarly, **Q1.16** and **Q1.17** *(corr = 0.63)*, **Q1.16** and **Q1.20** *(corr = 0.52)*, **Q1.20** and **Q1.28** *(corr = 0.52)* and **Q1.27** and **Q1.28** *(corr = 0.62)* are all have elements in common. Being the main boat-related source of information for one's friends, one's own sense of knowledge about boats, the frequency of boating related activities one performs and boating being someone's true passion in life and number one thing they do in their spare time are all characteristics that could be related. This again suggests that these 5 attributes could be combined.
* **Q1.3** and **Q1.9** *(corr = 0.58)* could also be combined, as suggested by the close relation (at least intuitively) between the two questions (brand in relation to identity and status symbol)
* Finally, we might want to investigate combining **Q1.6** and **Q1.7** *(corr = 0.55)*, as the two questions (reward for hard work and sense of achievement) are intuitively correlated.
* Further analysis is needed to determine the weight of each attribute when combined into a single group.
...
1. The PCA function gives us two measures of how much of any factor can explain the information: the percentage of variance explained and the eigenvalues of each component. The table lists the different attributes (uncorrelated) in order of percentage of variance explained. The sum of all eigenvalues (also from highest to lowest) equals the total number of components, in this case 28. The scree plot shows eigenvalues in descending order. Both the table and the plot can help us determine the optimal number of factors of combined variables we need, without losing too much of the captured data. Combining factors help us simplify and summarize our model, making it more interpretable and actionable.
2. In the plot, the descending curve is helpful to determine the "elbow" of the curve (where the values seem to converge), indicating the optimal number of factors that can be used. In this case, the intersection with the line = 1 tells us that we can theoretically choose only 5 components.
3. Since our goal is to find the smallest acceptable amount of factors to use, we should aim to keep all factors for which the eigenvalue is larger than 1, while ensuring that the cumulative percentage of variance explained remains above 50% (50% is an arbitrary value and needs to be further analysed in step 5). The table suggests that only 5 components are necessary. From the scree plot, we conclude that we theoretically could choose only 5 components. However, we need to understand how much of the data is lost by doing so, before moving forward, i.e., by understanding how the new data can be interpreted.
...
1. Because we've only selected 5 factors from a statistical perspective (eigenvalues >=1), the rotation shows 5 components. The first component includes the 6 questions that we have identified earlier as being similar to one another *(Q1.16, Q1.17, Q1.20, Q1.27, Q1.28)*. Additionally, all of the other questions with high correlation are also included in the first component. All 5 factors can be named accordingly:

    a. Identity and passion
    b. Adventurous lifestyle
    c. Quality and brand
    d. Knowledge
    e. Price  sensitivity/beginner 
    
    The relatively small number of components makes business sense, as it simplifies the interpretation of all answers and makes it easier to understand and analyse. However, since the **identity and passion** factor includes a lot of variables (11 or about 40% of all variables), there is some overlap between other components: **lifestyle** and **knowledge**. At a first glance, it seems like variables *Q1.16* and *Q1.17* may be more related to the **knowledge** component than to the **identity and passion** one. Similarly, Q1.7 would be closer to **lifestyle** than **identity and passion**. This is coherent with the weight of each of these variables relative to the highest weight of their respective factors. For example, *Q1.16* has a weight of 0.57, and the highest weight from **identity and passion** is 0.71 (*Q1.3*). Comparatively, *Q1.16* has a weight of 0.55 in **knowledge**, and the highest weight in that category is only 0.66 (*Q1.11*). Consequently, the overlap is not of high importance.
    
    Furthermore, an quick glance at each row and columns shows that only a few loadings are large in absolute values for all rows and columns. Additionnaly, all columns have a different pattern of loading

2. Selecting 5 components meant that the eigenvalues and scree plot analysis rules of thumb are respected. However, only about 50% of the cumulative variance explained was kept. Therefore, we will consider 5 as the minimum acceptable number of factors. Let's analyse for higher numbers:

    For 6 factors (55.84% of the cumulative variance explained), we have components that could be categorized into the following:
      a. Lifestyle
      b. Knowledge and passion
      c. Identity
      d. Quality and brand
      e. Price sensitivity/beginner
      f. Handson
      
    It is worth nothing that, while there is no overlap between any factor, the newly created **handson** factor only includes one variable. All of these components can be easily interpreted, but it may not make a lot of business sense to have one factor representing only one variable. In this case, we would favor simplicity over not having overlaps between factors.
      
    For 7 factors (58.65% of the cumulative variance explained), we get several components combining very few variables. Moreover, one component has combined two questions that intuitively should not be combined (Q1.10 When buying a boat, I rely more on expert opinion than my own and Q1.11 I tend to perform minor boat repairs and maintenance on my own). In fact, it seems like the model did not suppress Q1.10, which has a negative weight of -0.55. We therefore conclude that taking 7 factors is not advised.
    
    Considering that taking any number of factors above 7 will either give similar results or provide marginal improvement, we will only focus on either 5 or 6 factors. Our final choice lands on taking only 5 factors since we put a higher value to having a simple model in a business perspective (easier to explain to a manager), than to having strictly no overlap between factors (considering that this overlap is minimal).

3. We would interpret the factors selected as the following:
    a. Identity and passion
    b. Adventurous lifestyle
    c. Quality and brand
    d. Knowledge
...
4. From this analysis, we understand that models and R formulas are never perfect, and that a data scientist always needs to investigate and interpret the results. Depending on the end goal, for example, providing easier to understand data to a CEO, or favoring a higher accuracy and percentage of variance explained (say, cumulative of 90%), different number of factors can be chosen.
...
iprint.df(t(head(NEW_ProjectData, 20)), scale=TRUE)
...
1. Based on the first few observations of the new derived variables, it looks like **identity and passion** and **quality and brand** usually go hand in hand. In contrast, people that score high on those two factors do not show signs of **price sensitivity/beginner**.
*
*
*
*
